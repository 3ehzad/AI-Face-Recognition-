...
def drow_confusion_matrix(true,pred):
    ...
    plot_confusion_matrix(array, classes=class_names,title='Confusion matrix, without normalization')
...
def main(argv):
    ...
    subjects_path = "subjects.txt"
    accuracy = "low"

    splitFactor=0.3
    minFace=10
    enableConfusionMatrix="ON"

    #use following try except for passing argument through terminal
    try:
        opts, args = getopt.getopt(argv, "-train:-test:-name:-subjects:-accuracy:-split:-minface:-confm", ["training-data-path=", "test-data-path=", "name-flag=", "subjects-path=", "accuracy-level=","split-factor=","minface-face=","confm-confm="])
    ...
    for opt, arg in opts:
        print opt
        if opt == '-h':
            print "face.py --train <training data path> --test <test data path> --name <'True' or 'False'> --subjects <subject file path> --accuracy <'low' or 'high'> --split <number between 0 to 1 (0.3 is good)> --minface <int number> --confm <'ON' or 'OFF' "
            sys.exit()
