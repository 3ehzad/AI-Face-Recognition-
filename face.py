...
def detect_face(img,accuracy):
    ...
    while len(faces) != 1 and accuracy == "high":
        ...
        elif watch_dog_counter ==6 :
            min_Neighbors_counter=3
        elif 6 < watch_dog_counter <=10 :
...
def predict(test_img,face_recognizer,flag_name,subjects,accuracy,confusion,classifier_true,classifier_pred):
    ...
    if not face is None:
        global total_img_predicted
        total_img_predicted+=1
...
def main(argv):
    ...
    try:
        ...
        print 'face.py --train <training data path> --test <test data path> --name <True or False> --subjects <subject file path> --accuracy <low or high> --split <number between 0 to 1 (0.3 is good)> --minface <int number>'
        sys.exit(2)
    for opt, arg in opts:
        print opt
        if opt == '-h':
    ...
    if enableConfusionMatrix == "ON":
        plt.figure()
        print "total imgs that predicted = ",total_img_predicted
        print "total imgs in test = ", len(predicted_imgs)
