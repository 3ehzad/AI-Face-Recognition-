...
def detect_face(img,accuracy):
    ...
    while len(faces) != 1 and accuracy == "high":
        ...
        if watch_dog_counter <=5 :
            ...
            min_Neighbors_counter=3
            faces = face_cascade.detectMultiScale(gray, scaleFactor=1.001, minNeighbors=min_Neighbors_counter)
            min_Neighbors_counter += 1
        else:
            break
        watch_dog_counter+=1
...
def predict(test_img,face_recognizer,flag_name,subjects,accuracy):
...
def main(argv):
    ...
    flag_name = False
    training_data_path ="att_faces"
    test_data_path = "att_faces_test"
    subjects_path = "subjects.txt"
    accuracy = "low"

    #use following try except for passing argument through terminal
    try:
        opts, args = getopt.getopt(argv, "-train:-test:-name:-subjects:-accuracy:", ["training-data-path=", "test-data-path=","name-flag=","subjects-path=","accuracy-level="])
    ...
    predicted_imgs = []
    for i in test_imgs:
        predicted_imgs.append(predict(i,face_recognizer,flag_name,subjects,accuracy))
